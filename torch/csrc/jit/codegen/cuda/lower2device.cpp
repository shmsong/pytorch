...
class Buffers : OptOutDispatch {
 public:
  ...
  static std::vector<kir::Allocate*> getDynamicAllocs(
      const std::vector<Expr*>& exprs) {
    Buffers fgr;
    for (auto expr : exprs) {
      fgr.handle(expr);
    }
    return fgr.dynamic_allocations_;
  }

  static std::vector<kir::Allocate*> getStaticAllocs(
      const std::vector<Expr*>& exprs) {
    Buffers fgr;
    for (auto expr : exprs) {
      fgr.handle(expr);
    }
    return fgr.static_allocations_;
  }
...
void GpuLower::lower() {
  ...
  // Get allocations
  global_allocations_ = Buffers::getGlobalAllocs(lowered_exprs_);
  sync_allocations_ = Buffers::getSyncAllocs(lowered_exprs_);
  dynamic_smem_allocations_ = Buffers::getDynamicAllocs(lowered_exprs_);
  static_smem_allocations_ = Buffers::getStaticAllocs(lowered_exprs_);
}
