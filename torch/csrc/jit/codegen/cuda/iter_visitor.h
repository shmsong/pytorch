...
class TORCH_CUDA_API IterVisitor : public OptOutDispatch {
 ...
 protected:
  ...
  // multiple times.
  virtual inline std::vector<Statement*> next(Statement* stmt) {
    if (stmt->isVal()) {
  ...
  virtual inline std::vector<Statement*> next(Val* v) {
  ...
  virtual inline std::vector<Statement*> next(Expr* expr) {
